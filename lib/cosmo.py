...
class BaseData:
    ...
    def _get_ds(self, suffix, filter_keys, *, name="") -> xarray.Dataset:
        ...
        if fileprefix == "lgfff":
            suffix += ".grb"
            if filter_keys["typeOfLevel"] == "generalVerticalLayer":
                suffix = ".grb"
        if name:
...
class ModelData(BaseData):
    ...
    def time(self) -> ModelParam:
        return ModelParam("sw", level_type="surface", param_name="time")
    ...
    def pmsl(self) -> ModelParam:
        return ModelParam("sw", level_type="meanSea", param_name="PMSL")

    @property
    def clcm(self) -> ModelParam:
        return ModelParam("sw", level_type="isobaricLayer", param_name="CLCM", level=400)

    @property
    def tot_prec(self) -> ModelParam:
        return ModelParam("sw", level_type="surface", param_name="TOT_PREC")
    ...
    def dhail_avg(self) -> ModelParam:
        return ModelParam("hail", level_type="surface", param_name="DHAIL_AV", step_type="avg")
    ...
    def u_10m(self) -> ModelParam:
        return ModelParam("sw", level_type="heightAboveGround", param_name="U_10M", level=10)

    @property
    def v_10m(self) -> ModelParam:
        return ModelParam("sw", level_type="heightAboveGround", param_name="V_10M", level=10)
...
class ComputedModelData(ModelData):
    ...
    def dbz(self, lead_time) -> numpy.ndarray:
        return self._concatenate_values(lead_time, 'dbz_ctmax').values
