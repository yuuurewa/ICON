...
class BasePlot:
    ...
    def create(self, title):
        ...
        for c in self.cities:
            ...
            self.ax.plot(c['lon'], c['lat'], marker='o', color='red', markersize=3, alpha=1,
                     transform=self.transform)  # transform=crs.Geodetic())
    ...
    def draw_contourf(self, geom, lats, lons, levels, *, cm=None, cmap_list=None, extend='both', opacity=0.7):
        if cmap_list:
    ...
    def draw_colorbar(self, c, cbar, levels):
        if "cax" in cbar.keys():
            ...
            colorbar = self.fig.colorbar(c, ax=self.ax, orientation=cbar["orientation"], ticks=levels)
        colorbar.set_label(cbar["label"])
        colorbar.ax.tick_params(labelsize=9)

    def draw_contour(self, geom, lats, lons, levels, color, linewidth=2, linestyles='solid'):
        c = self.ax.contour(
            lons, lats, geom, 0,
            colors=color,
        ...
        self.ax.clabel(c, c.levels, fmt="%d", inline=True, fontsize=10)
        return c
    ...
    def draw_barbs(self, u, v, lats, lons):
        ...
        self.ax.barbs(lons[::scale, ::scale], lats[::scale, ::scale], u[::scale, ::scale], v[::scale, ::scale],
                  ...
                  length=4, sizes=dict(emptybarb=0.05, spacing=0.2, height=0.7), alpha=0.6)
    ...
    def save(self, name, image_type="png"):
        ...
        plt.figtext(0.9, 0.125, "©СибНИГМИ", ha="right", fontsize=11)
        if image_type == "tiff":
