...
parameters = ("stp", "precipitation", "wind_gust", "dbz", "t2m")
    ...
    for lead_time_min in model_fileset(fc_start_minutes, fc_end_minutes, data_step_minutes):
        ...
        with ProcessPoolExecutor(len(parameters)) as executor:
            ...
            for parameter in parameters:
                plot.def_map(Map6km(IMAGE_DIR, date, "cosmo_phenom"))
                print(parameter)
...
def main():
    ...
    aggregation_hours = [0, 24, 48, 72]
    fc_start_minutes = 3 * 60
    fc_end_minutes = 78 * 60 + 1
