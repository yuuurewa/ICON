...
if os.environ.get('HOSTNAME') == "xfront2" and sys.argv[1]:
    ...
    date = "2023110800"
DATA_DIR, IMAGE_DIR = set_paths(f"{NAME}_LHN", resolution, date)

resolution = float(resolution) / 10


parameters = ("stp", "precipitation", "wind_gust", "dbz", "lpi")
parameters_agg = ("gust_max", "lpi_max24", "sdi2_max24", "hail_max", "precip_sum")
    ...
    for lead_time_min in model_fileset(fc_start_minutes, fc_end_minutes, data_step_minutes):
        ...
        with ProcessPoolExecutor(len(parameters)) as executor:
            ...
            for parameter in parameters:
                plot.def_map(Map2km(IMAGE_DIR, date, "cosmo_lhn"))
                print(parameter)
                futures.append(executor.submit(getattr(plot, parameter), fc_time, lead_time))
            for future in as_completed(futures):
                future.result()

    with ProcessPoolExecutor(len(parameters_agg)) as executor:
        futures = []
...
def main():
    ...
    select_path(DATA_DIR, "lfff")

    aggregation_hours = [0, 23]
    fc_start_minutes = 1 * 60
    fc_end_minutes = 12 * 60 + 1
